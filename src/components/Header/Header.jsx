			...
			{!isMobile ? (
				<>
					{!isMenu ? (
						<header
							...
							<Link to='/'>
								<img src={isChange ? logoImageHover : logoImage} alt="브레인시티 푸르지오 -mainLogo-image" />
							</Link>
				...
				<div className={styles.mobileHeader}>
					<div onClick={() => setIsMobileMenu(!isMobileMenu)}>
						{!isMobileMenu ?
							<AiOutlineMenu className={styles.icon} size={25} color="#890d18" />
							:
							<IoCloseSharp className={styles.icon} size={25} color="#890d18" />
						}
					</div>
					...
					<a href={'tel:1533-8848'}>
						<IoCall className={styles.icon} size={25} color="#890d18" />
					</a>
				</div>
			)}
		</>
